...
{
    public sealed class DriverValidatorUpdate : Validator<Driver>
    {
        private readonly IEnumerable<Driver> _items;

        public DriverValidatorUpdate(IEnumerable<Driver> items)
        {
            _items = items;
            RuleFor(x => x).NotNull();
        ...
        {
            return _items.Any(t =>
              t.FirtName.ToLower() == driver.FirtName.ToLower() && t.LastName.ToLower() == driver.LastName.ToLower() && t.Id != driver.Id);
        }
    }
}
