...
namespace Wappa.Framework.Google.Geocoding
{
    ...
    {
        private const string keyMessage = "Only one of BusinessKey or ApiKey should be set on the GeocoderService.";
        ...
        async Task<IEnumerable<Address>> IGeocoderService.GeocodeAsync(string address)
        {
            return await GeocodeAsync(address).ConfigureAwait(false);
        }

        async Task<IEnumerable<Address>> IGeocoderService.GeocodeAsync(string street, string city, string state, string postalCode, string country)
        {
